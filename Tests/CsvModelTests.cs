...
c[0].id
    ...
    [TestMethod]
    public void SerialisationOverflowIsCaught()
    {
        var template = @"{{
...
";
        var error = new ApplicationEngine(new RunTimeEnvironment(_files))
            .WithHelpers()
            .WithTemplate(template)
            .RenderToErrorOrOutput();

        error.Should().Contain("error : Attempt to serialise deeply nested or recursive structure");
    }
}
